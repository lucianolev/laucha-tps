...
Como $G$ no es completo pero si conexo, existen dos nodos $a$ y $z$ tal que no son adyacentes y existe un camino entre ellos. Sea $b$ un nodo adyacente a $a$ que pertenece al camino entre $a$ y $z$. $b$ es distinto de $z$ ya que si fueran iguales $a$ y $z$ ser\'ian adyacentes y eso es absurdo por hipotesis. Ahora tomamos un nodo adyacente a $b$ que tambi\'en pertenezca al camino entre $a$ y $z$ al que llamaremos $c$. Este nodo puede ser igual a $z$ (en caso de que el camino tenga longitud dos) o un nodo intermedio en el camino entre $a$ y $z$. Si $c$ es igual a $z$ entonces me puedo formar un $K_{3}-1$ ya que $a$ es adyacente a $b$, $b$ es adyacente a $z$ y $z$ no es adyacente a $a$ por hipotesis (ver Figura1). Si $c$ es distinto de $z$ entonces tenemos dos casos: si $c$ no es adyacente a $a$ entonces podemos formar un $K_{3}-1$ con dichos nodos (ver Figura2). Si $c$ si es adyacente a $a$, entonces no podemos formar un $K_{3}-1$ entre $a$ $b$ y $c$ (ver Figura3). En este \'ultimo caso tomamos como nuevo nodo $b$ a $c$ (es adyacente a $a$ y pertenece al camino entre $a$ y $z$) y como nuevo nodo $c$ a un nodo adyacente a $c$ que pertenezca al camino entre $a$ y $z$ y todavia no haya sido recorrido. Nuevamente caemos en el caso anterior, donde la posibilidad de poder armar un $K_{3}-1$ depende de si $c$ es adyacente o no a $a$. Si no es posible obtener un $K_{3}-1$ con el nodo $c$ elegido (debido a que $c$ es adyacente a $a$), seguiremos avanzando los nodos $b$ y $c$ pertenecientes al camino entre $a$ y $z$ hasta llegar a que $b$ es el nodo anterior a $z$ del camino y $c$ es igual a $z$. En este caso, siempre es posible armar un $K_{3}-1$ ya que $z$ no es adyacente a $a$ por hipotesis (ver Figura4).
...
Sea $G$ un grafo que contiene un subgrafo diamante como subgrafo inducido. Sea $G_{D}$ el subgrafo diamante de $G$ (Figura1). Si tomo uno de los dos nodos de $G_{D}$ de grado tres, que llamaremos $v$, su vecindad $N_{G_{D}}(v)$ sera: (Figura2). Podemos ver entonces que $N_{G_{D}}(v)$ tiene una \'unica componente conexa no completa ya que los dos nodos de grado dos del diamante no son adyacentes.
...
Como se mencion� anteriormente, este algoritmo est� basado en la propiedad demostrada en el punto a, es decir, para cada nodo del grafo estudia las componentes conexas de la vecindad de dicho nodo para determinar si existe un diamante. En primer lugar, se eliminan todos los nodos de grado 0 � 1, ya que estos no pueden formar parte de ning�n diamante y eliminarlos no afecta a ning�n diamante del grafo. Esto se debe a que cualquier nodo que pertenezca a un diamante tiene por lo menos grado 2. En segundo lugar, se crean la lista de adyacencias (que llamaremos \textit{adyacencias}) y la matriz de adyacencia, que resultar�n necesarias para calcular la vecindad de cada nodo y verificar si dos nodos son adyacentes, respectivamente. Luego, el algoritmo comienza a chequear por cada nodo (en adelante llamaremos superNodo) su vecindad para encontrar componentes conexas no completas y as� determinar donde se encuentra el diamante. La funci�n \textit{crearVecindad} se encarga de construir la vecindad (que es un grafo representado con una lista de adyacencia) de cada superNodo a partir de la lista \textit{adyacencias}, es decir, por cada elemento k de la lista de adyacencia de superNodo se recorre la lista de adyacencia del elemento k agregando en la vecindad los elementos que est�n relacionados con superNodo.

Una vez obtenida la vecindad (vecindadDeSuperNodo), se comienza a revisar las componentes conexas de la misma para verificar si son completas a trav�s de la funci�n \textit{buscarDiamanteMinimoEnVecindad}. Con este fin, se utiliza Depth First Search (DFS) para recorrer cada componente conexa almacenando la suma de los grados de cada nodo recorrido as� como el nodo m�nimo encontrado (nodoMinimoCompConexa) y la lista de nodos por los que se avanz� (nodosCompConexa). Al terminar de recorrer cada componente conexa, se verifica si la misma es completa o no. En caso de que no lo sea, se sabe que ninguno de los nodos de esa componente conexa forma un diamante\footnotemark[1], por lo que se descarta y contin�a verificando las siguientes componentes. En caso afirmativo, se procede a encontrar los nodos que forman el diamante\footnotemark[1] m�nimo para esa componente conexa por medio de la funci�n \textit{buscarDiamanteMinimoDeCompConexa}. Esta funci�n utiliza el nodo m�nimo de la componente conexa (nodoMinimoCompConexa) recorrida para obtener el diamante. Es decir, toma la lista de adyacencia de nodoMinimoCompConexa y para cada nodo k de dicha lista, recorre todos los elementos j de su lista de adyacencia para determinar si forma un $K_{3}-1$ entre nodoMinimoCompConexa, $k$ y $j$. En este caso, se sabe que los elementos encontrados forman un diamante con superNodo, ya que superNodo es adyacente a los mismos pues pertenecen a su vecindad. Como la funci�n \textit{buscarDiamanteMinimoDeCompConexa} verifica en cada paso si el diamante obtenido es menor al que se tiene hasta ese momento y se prueban todas las posibles diamantes constru�dos a partir de nodoMinimoCompConexa, se puede afirmar que obtiene el diamante m�nimo para dicha componente conexa. Esto se debe a que nodoMinimoCompConexa es el menor nodo que puede formar un diamante\footnotemark[1] y los restantes nodos se obtienen seleccionando el menor de todos los posibles diamantes.

Luego, dentro de la funci�n \textit{buscarDiamanteMinimoEnVecindad} se verifica si el diamante m�nimo de la componente conexa actual es menor que el diamante que se tiene hasta ese paso (almacenado en diamanteMinimo). Si esto sucede, el diamante de la componente conexa actual pasa a ser el diamante m�nimo. Una vez verificadas todas las componentes conexas se obtiene, en diamanteMinimo, el m�nimo diamante para la vecindad de superNodo. Este diamante se almacena en una lista llamada diamantesMinimos que, cuando el algoritmo termina de verificar las vecindades de todos los nodos, contendr� los diamantes m�nimos para cada una de las mismas, en caso de que este exista. Finalmente, se busca el m�nimo de la lista diamantesMinimos, con lo que se obtiene el diamante m�nimo de todo el grafo, resolviendo as� el problema.

\footnotetext[1]{Las demostraciones de estas propiedades fueron presentadas en la secci�n ``Demostraci�n de la parte a''}
