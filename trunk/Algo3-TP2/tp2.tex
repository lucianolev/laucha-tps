...
En la presente secci\'on se calcular\'a la complejidad en el peor caso del algoritmo \textit{fumigar} bas\'andose en el modelo de c\'omputo uniforme. Para esto se dividir\'a el analisis en el estudio de las dos partes de la funci\'on: \textit{fumigaci\'on} y \textit{c\'alculo de litros por zona}
...
Finalmente, como la complejidad de la primer parte de la funci\'on pertenece a $O(nm^{2})$, la segunda pertenece a $O(n+m)$ y $O(n+m)\ \in O(nm^{2})$, concluimos que la complejidad en peor caso del algoritmo \textit{fumigar} es $$T(n,m) \in O(nm^{2})$$

\bigskip

%TODO: en funcion del tamaño de entrada
